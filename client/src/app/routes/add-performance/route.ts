...
const POST = async (request: NextRequest) => {
    try {
        ...
        const apiResponse = await fetch("http://localhost:8000/add-performance", {
            method: "POST",
            headers: {
        ...
        if (!apiResponse.ok) {
            ...
            return NextResponse.json(
                { detail: errorData.detail || "Failed to add performance" },
        ...
        return catchError(error,
            "Error caught in /add-performance/route.ts. Reason: ",
            "Unknown error caught in /add-performance/route.ts."
        );
    }
}
